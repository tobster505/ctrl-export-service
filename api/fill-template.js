...
const N = (v, fb = 0) => { const n = Number(v); return Number.isFinite(n) ? n : fb; };
const norm = (v, fb = '') =>
  S(v, fb).replace(/[\u2018\u2019]/g,"'").replace(/[\u201C\u201D]/g,'"').replace(/[\u2013\u2014]/g,'-')
...
function formatDateLbl(d) {
  try {
    const parts = new Intl.DateTimeFormat('en-GB', { timeZone:'Europe/Amsterdam', day:'2-digit', month:'short', year:'numeric' }).formatToParts(d);
...
const pickFullName = (data, url) => norm(
  data?.person?.fullName ?? data?.fullName ?? data?.summary?.user?.fullName ?? qstr(url,'full','')
);
const pickCoverName = (data, url) => norm(
  data?.person?.coverName ?? data?.coverName ?? data?.summary?.user?.reportCoverName ?? pickFullName(data,url)
);
function pickFlowLabel(data, url) {
  const raw = (data?.flowLabel ?? data?.PathName ?? data?.summary?.flow?.label ?? qstr(url,'flow','Perspective')).toString().toLowerCase();
...
export default async function handler(req, res) {
    ...
    data = {
      ...
      chartUrl:'https://quickchart.io/chart?v=4&c=' + encodeURIComponent(JSON.stringify({
        ...
        data:{ labels:['Concealed','Triggered','Regulated','Lead'],
          datasets:[{ label:'Frequency', data:[0,2,3,0], fill:true,
      ...
      dominantParagraph:'You connect the most with Mika — measured, fair, and steady under pressure.'
    };
  } else {
  ...
  const POS = {
    // Page 1 — PathName + FullName + Date (locked per your coords)
    n1: { x:-10, y:570, w:600, size:32, align:'center', color:rgb(0.12,0.11,0.2) },  // Path
    f1: { x:285, y:165, w:400, size:40, align:'left',   color:rgb(0.12,0.11,0.2) },  // Full
    d1: { x:120, y:630, w:500, size:25, align:'left',   color:rgb(0.12,0.11,0.2) },  // Date

    // Page 2 header (locked)
    f2: { x:200, y:64, w:400, size:13, align:'left',   color:rgb(0.12,0.11,0.2) },
    n2: { x: 25, y:64, w:800, size:12, align:'center', color:rgb(0.12,0.11,0.2) },

    // Pages 3..8 headers (defaults; all tunable via URL)
    ...
    // Page 5 — Dominant, desc, how, chart, patterns (moved), themes (moved)
  ...
  // Pages 3..8 headers (this is where f3*/n3* etc are read)
  for (let p = 3; p <= 8; p++) {
    const fk = `f${p}`, nk = `n${p}`;
  ...
  if (debug) {
    try {
      ...
      res.end(JSON.stringify({ ok:true, numPages, pos:POS, urlParams:Object.fromEntries(url.searchParams.entries()) }, null, 2));
    } catch (e) {
      res.setHeader('Content-Type','application/json');
  ...
  try {
    ...
    const P = i => (i >= 0 && i < pages.length ? pages[i] : null);

    const page1 = P(0), page2 = P(1), page3 = P(2), page4 = P(3),
          page5 = P(4), page6 = P(5), page7 = P(6), page8 = P(7);
    ...
    if (page5) {
      ...
      if (!noGraph && data?.chartUrl) {
        try {
          const r = await fetch(S(data.chartUrl,'')); if (r.ok) {
            const png = await pdf.embedPng(await r.arrayBuffer());
            const { x,y,w,h } = POS.c5; const ph = page5.getHeight();
            page5.drawImage(png, { x, y: ph - y - h, width:w, height:h });
          }
        } catch { /* ignore */ }
      }

      // Patterns (left) — moved to page 5
      const rawBlocks = Array.isArray(data.page2Patterns) ? data.page2Patterns
                        : Array.isArray(data.page2Blocks) ? data.page2Blocks : [];
      const twoBlocks = rawBlocks.map(b=>({ title:norm(b?.title||''), body:norm(b?.body||'') }))
                                 .filter(b=>b.title||b.body).slice(0,2);
      let curY = POS.p2p.y;
      for (const b of twoBlocks) {
        if (b.title) {
          drawTextBox(page5, HelvB, b.title,
        ...
        if (b.body) {
          const r = drawTextBox(page5, Helv, b.body,
    ...
    // Page 6
    if (page6) { if (flowLabel) drawTextBox(page6, HelvB, flowLabel, { ...POS.n6 }); if (fullName) drawTextBox(page6, Helv, fullName, { ...POS.f6 }, { maxLines:1, ellipsis:true }); }
    // Page 7
    if (page7) { if (flowLabel) drawTextBox(page7, HelvB, flowLabel, { ...POS.n7 }); if (fullName) drawTextBox(page7, Helv, fullName, { ...POS.f7 }, { maxLines:1, ellipsis:true }); }
    // Page 8 (optional)
    if (page8) { if (flowLabel) drawTextBox(page8, HelvB, flowLabel, { ...POS.n8 }); if (fullName) drawTextBox(page8, Helv, fullName, { ...POS.f8 }, { maxLines:1, ellipsis:true }); }
