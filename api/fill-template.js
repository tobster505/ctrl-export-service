...
function drawTextBox(page, font, text, spec = {}, opts = {}) {
  ...
  const hard = norm(text || ""); // <- WinAnsi safety
  if (!hard) return;
...
function normaliseInput(d = {}) {
  ...
  return {
    ...
    spiderdesc: norm(d.spiderdesc || d.spider || d["p4:spiderdesc"] || ""),
    spiderfreq: norm(d.spiderfreq || d["p4:spiderfreq"] || ""),
    seqpat:     norm(d.seqpat || d.seqat || d.pattern || ""),
    theme:      norm(d.theme || d.themes?.text || ""),
...
export default async function handler(req, res) {
  try {
    ...
    // Decode payload (accept base64 + base64url; tolerate URL-enc)
    const src = parseDataParam(q.data);
    // If a separate ?chart=… param is provided, honor it unless data already has chartUrl
    if (q.chart && !src.chart && !src.chartUrl) src.chart = q.chart;
    ...
    // p4 (description + chart)  ——— THIS IS THE KEY FIX
    ...
    // Optional: frequency line if your layout includes it
    if (L.p4?.spiderFreq && P.spiderfreq) {
      drawTextBox(p(3), font, P.spiderfreq, L.p4.spiderFreq, { maxLines: L.p4.spiderFreq.maxLines || 1 });
    }
    ...
    // p11 (split: 2 tips + 2 actions) — bullets with indentation
    if (L.p11?.split) {
      ...
      const bulletIndent = N(L.p11.bulletIndent, 18);
      const drawIndented = (pg, s, b) => {
        if (!s || !b) return;
        // bullet + wrap: indent first line by 0, subsequent by bulletIndent
        const body = norm(s);
        const firstPrefix = "- ";
        const widthOf = (t, size) => font.widthOfTextAtSize(t, Math.max(1, size));
        const words = body.split(/\s+/);
        let lines = [];
        let cur = "";
        for (const w of words) {
          const next = cur ? `${cur} ${w}` : w;
          const maxW = (cur ? (b.w - bulletIndent) : (b.w - widthOf(firstPrefix, b.size||18)));
          if (widthOf(next, b.size||18) <= maxW || !cur) cur = next;
          else { lines.push(cur); cur = w; }
        }
        if (cur) lines.push(cur);
        lines = lines.slice(0, b.maxLines || 4);
        const pageH = p(10).getHeight();
        let y = pageH - b.y - (b.size||18); // baseline first line
        for (let i=0; i<lines.length; i++){
          const isFirst = (i===0);
          const prefix = isFirst ? firstPrefix : "";
          const x = b.x + (isFirst ? 0 : bulletIndent);
          p(10).drawText(prefix + lines[i], { x, y, size: b.size||18, font, color: rgb(0,0,0) });
          y -= (b.size||18) + (L.p11.lineGap||6);
        }
      };
      for (const { txt, box } of pairs) drawIndented(p(10), txt, box);
    }
