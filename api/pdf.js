...
export default async function handler(req, res) {
  try {
    ...
    if (seqLine) { page.drawText(seqLine, { x: margin, y, size: 11, font }); y -= 16; }
    const c = p.counts || {};
    const countsLine = `Counts — C:${c.C||0}  T:${c.T||0}  R:${c.R||0}  L:${c.L||0}`;
    page.drawText(countsLine, { x: margin, y, size: 11, font }); y -= 18;
    ...
    for (const b of blocks) {
      if (!b.text) continue;
      page.drawText(`${b.label}:`, { x: margin, y, size: 12, font: fontB });
      y -= 16;
      for (const ln of wrapText(b.text, 98)) {
    ...
    // Footer
    page.drawText("CTRL • Generated by /api/pdf", { x: margin, y: 30, size: 9, font, color: rgb(0.4,0.4,0.4) });
