// /api/pdf-html.js  — Render HTML with CSS, print to A4 PDF via headless Chrome (Vercel)
// ESM
...
function renderHTML(p) {
  ...
  const counts = p.raw?.counts
    ? `C:${p.raw.counts.C ?? 0}  T:${p.raw.counts.T ?? 0}  R:${p.raw.counts.R ?? 0}  L:${p.raw.counts.L ?? 0}`
    : squash(p.raw?.counts ?? '');

  // Inline CSS, single-hue palette
  return `<!doctype html>
<html>
<head>
<meta charset="utf-8"/>
...
<style>
  :root{
    --ink:#2E2A36;
    --muted:#5C566C;
    --accent:#7348C7;      /* mauve-500 */
    --accent2:#9D7BE0;     /* mauve-400 */
    --box:#F5F2FB;
    --box-stroke:#E2DAF6;
    --tip:#EFE7FF;
    --tip-stroke:#D7C7FB;
  ...
  *{ box-sizing:border-box; }
  ...
  .card h3{
    font-size:12px; margin:0 0 4px; color:var(--accent); font-weight:700;
  }
  ...
  .chart .imgwrap{ display:flex; align-items:center; justify-content:center; width:100%; height:220px; padding:10px; }
  ...
  .tip{
    background:var(--tip); border:1px solid var(--tip-stroke);
    border-radius:12px; padding:12px 14px;
  }
  .tip h4{ margin:0 0 6px; font-size:12px; color:var(--accent); }
  .tip p{ font-size:11px; margin:0; }
  /* Footer */
